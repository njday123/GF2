...
//Tested successfully. Prints out numbers and names from a text file.
...
void skipInvalidChars(ifstream *infp, char &curch, bool &eofile);
...
// be compiled with the command  g++ -g -o small small.cc
{
  ...
  while (!eofile) {
    skipInvalidChars(&inf, ch, eofile);
	if (!eofile){
		if (isDigit(ch)){
...
void skipInvalidChars(ifstream *infp, char &curch, bool &eofile){
	while (!eofile) {
		if (!(isLetter(curch) || isDigit(curch))){
			//Character is not a letter/number. Get the next char from the input stream.
			eofile = !(*infp).get(curch); 
		}else{	
			//Character is a letter/number. Return program flow to calling procedure.
			return;
		}
	}
}
